...
int indent; /* indent for inside conditionals */
{
  ...
  for (;;)
  {
    ...
    if (xpc.xp_context == EXPAND_FILES && p_wmnu)
    {
      ...
      if (ccline.cmdbuff[ccline.cmdpos - 1] == PATHSEP && c == K_DOWN && (ccline.cmdbuff[ccline.cmdpos - 2] != '.' || ccline.cmdbuff[ccline.cmdpos - 3] != '.'))
      {
        /* go down a directory */
        c = p_wc;
      }
      ...
      else if (c == K_UP)
      {
        ...
        while (--j > i)
        {
          ...
          if (vim_ispathsep(ccline.cmdbuff[j])
              ...
              && vim_strchr(" *?[{`$%#", ccline.cmdbuff[j + 1]) == NULL
#endif
          )
          {
            if (found)
    ...
    if (c == Ctrl_BSL)
    {
      ...
      if (c != Ctrl_N && c != Ctrl_G && (c != 'e' || ccline.cmdfirstc == '='))
      {
        vungetc(c);
        c = Ctrl_BSL;
      }
    ...
    {
      if (nextwild(&xpc, WILD_EXPAND_KEEP, 0) == OK && nextwild(&xpc, WILD_PREV, 0) == OK && nextwild(&xpc, WILD_PREV, 0) == OK)
        goto cmdline_changed;
    }
...
#ifdef FEAT_MBYTE
        if (has_mbyte)
          ccline.cmdpos += (*mb_ptr2len_check)(ccline.cmdbuff + ccline.cmdpos);
        else
#endif
          ++ccline.cmdpos;
      } while ((c == K_S_RIGHT || c == K_C_RIGHT) && ccline.cmdbuff[ccline.cmdpos] != ' ');
#ifdef FEAT_MBYTE
      if (has_mbyte)
        set_cmdspos_cursor();
#endif
    ...
    case K_C_LEFT:
      ...
      } while ((c == K_S_LEFT || c == K_C_LEFT) && ccline.cmdbuff[ccline.cmdpos - 1] != ' ');
#ifdef FEAT_MBYTE
      if (has_mbyte)
        set_cmdspos_cursor();
#endif
    ...
    case K_RIGHTMOUSE:
           ...
           ++ccline.cmdpos)
      {
        ...
        if (has_mbyte)
        {
          ...
          ccline.cmdpos += (*mb_ptr2len_check)(ccline.cmdbuff + ccline.cmdpos) - 1;
        }
#endif
        ccline.cmdspos += i;
      }
    ...
    case Ctrl_P: /* previous match */
      ...
      {
        if (nextwild(&xpc, (c == Ctrl_P) ? WILD_PREV : WILD_NEXT, 0) == FAIL)
          break;
        goto cmdline_changed;
      }
    ...
    case K_KPAGEDOWN:
      ...
      for (;;)
      {
        ...
        if ((c != K_UP && c != K_DOWN) || hiscnt == i || STRNCMP(history[histype][hiscnt].hisstr, lookfor, (size_t)j) == 0)
          break;
      }
 ...
 */
static void
    correct_cmdspos(idx, cells) int idx;
int cells;
{
  if ((*mb_ptr2len_check)(ccline.cmdbuff + idx) > 1 && (*mb_ptr2cells)(ccline.cmdbuff + idx) > 1 && ccline.cmdspos % Columns + cells > Columns)
    ccline.cmdspos++;
}
#endif
...
redrawcmd_preedit()
{
  if ((State & CMDLINE) && xic != NULL && im_get_status() && !p_imdisable && im_is_preediting())
  {
    int cmdpos = 0;
    int cmdspos;
    int old_row;
 ...
 */
static void
    draw_cmdline(start, len) int start;
int len;
{
#if defined(FEAT_CRYPT) || defined(FEAT_EVAL)
  int i;
  ...
  else
  {
    ...
    else
    {
      p2 = ExpandOne(xp, p1, vim_strnsave(&ccline.cmdbuff[i], oldlen),
                     WILD_HOME_REPLACE | WILD_ADD_SLASH | WILD_SILENT | WILD_ESCAPE | options, type);
      vim_free(p1);
  ...
  if (mode == WILD_LONGEST && xp->xp_numfiles > 0)
  {
    for (len = 0; xp->xp_files[0][len]; ++len)
    {
      for (i = 0; i < xp->xp_numfiles; ++i)
      {
        ...
        if (xp->xp_context == EXPAND_DIRECTORIES || xp->xp_context == EXPAND_FILES || xp->xp_context == EXPAND_BUFFERS)
        {
...
int options;
{
  ...
  {
    if (xp->xp_context == EXPAND_FILES || xp->xp_context == EXPAND_BUFFERS || xp->xp_context == EXPAND_DIRECTORIES)
    {
      /*
...
#define L_SHOWFILE(m) (showtail ? sm_gettail(files_found[m]) : files_found[m])
  ...
  else
  {
    ...
    for (i = 0; i < num_files; ++i)
    {
      if (!showtail && (xp->xp_context == EXPAND_FILES || xp->xp_context == EXPAND_BUFFERS))
      {
    ...
    for (i = 0; i < lines; ++i)
    {
      ...
      for (k = i; k < num_files; k += lines)
      {
        ...
        for (j = maxlen - lastlen; --j >= 0;)
          msg_putchar(' ');
        if (xp->xp_context == EXPAND_FILES || xp->xp_context == EXPAND_BUFFERS)
        {
...
int context; /* EXPAND_FILES etc. */
{
  ...
  if (context != EXPAND_FILES && context != EXPAND_DIRECTORIES)
  {
    ...
    if (context == EXPAND_HELP || context == EXPAND_COLORS || context == EXPAND_COMPILER || (context == EXPAND_TAGS && fname[0] == '/'))
    ...
    else
    {
      ...
      if (retval != NULL)
      {
        ...
        for (i = 0; i < len; i++, j++)
        {
           ...
           * expansion. */
          if (context != EXPAND_USER_DEFINED && fname[i] == '\\' && ++i == len)
            break;
  ...
  else
  {
    ...
    if (retval != NULL)
    {
       ...
       */
      tail = gettail(retval);
      if ((*retval != '~' || tail != retval) && vim_strchr(tail, '$') == NULL && vim_strchr(retval, '`') == NULL)
        retval[len++] = '*';
      retval[len] = NUL;
    }
  }
...
static void
    cleanup_help_tags(num_file, file) int num_file;
char_u **file;
{
  int i, j;
  int len;
 ...
 */
int ExpandGeneric(xp, regmatch, num_file, file, func)
expand_T *xp;
regmatch_T *regmatch;
int *num_file;
char_u ***file;
...
char_u *file;
{
  ...
  while (*path != NUL)
  {
    ...
    if (STRLEN(buf) + STRLEN(file) + 2 < MAXPATHL)
    {
      add_pathsep(buf);
      STRCAT(buf, file);
      if (ExpandFromContext(&xpc, buf, &num_p, &p, WILD_SILENT) != FAIL && num_p > 0)
      {
 ...
 */
void
    add_to_history(histype, new_entry, in_map, sep) int histype;
char_u *new_entry;
...
int num;
{
  ...
  if (hislen == 0 || histype < 0 || histype >= HIST_COUNT || (i = hisidx[histype]) < 0 || num == 0)
    return -1;
...
char_u *str;
{
  ...
  if (hislen != 0 && histype >= 0 && histype < HIST_COUNT && *str != NUL && (idx = hisidx[histype]) >= 0 && (regmatch.regprog = vim_regcomp(str, RE_MAGIC + RE_STRING)) != NULL)
  {
    i = last = idx;
    do
    {
 ...
 */
void
    prepare_viminfo_history(asklen) int asklen;
{
  int i;
  int num;
  int type;
